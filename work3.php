...
$_SESSION['arr'] = [1, '2', 'test']; // Пулучаются массивы с вложеным массивом
...
var_dump(session_destroy()); // Если destroy отработает мы получим 1, если не отработает, получим 0
...
// Удаление куки
if (ini_get("session.use_cookies")) { // Запускается главная проверка, если пройде true,
// то пойдет выполнение всей команды, конкретно, если есть конкретные куки этой сессии

    $params = session_get_cookie_params(); // Создаем переменную и присваеваем все куки
// которые относятся к данной сессии

    setcookie(session_name(), '' , time() - 42000, // Создаем новое пустое имя куки
